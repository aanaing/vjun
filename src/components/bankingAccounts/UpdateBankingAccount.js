...
import { UPDATE_BANKING_ACCOUNT } from '../../gql/banking_accounts'

import { Box, Card, CardContent, FormControl, TextField, Typography, Button, } from '@mui/material'
import { LoadingButton } from '@mui/lab';

const UpdateBankingAccount = ({ handleClose, bankingAlert, account }) => {
    ...
    const [ updateAccount ] = useMutation(UPDATE_BANKING_ACCOUNT, {
        onError: (error) => {
        ...
        onCompleted: () => {
            ...
            setLoading(false)
            bankingAlert('Banking Account have been updated')
            handleClose()
        },
    })
    ...
    useEffect(() => {
        if(account) {
            setValues({ account_number: account.account_number, service_name: account.payment_service_name, receiver_name: account.receiver_name })
        }
    }, [account])
    ...
    const handleCreate = async () => {
        ...
        try {
            updateAccount({variables: { ...values, id: account.id }})
        } catch (error) {
            console.log('error : ', error)
        }
    }
...
export default UpdateBankingAccount