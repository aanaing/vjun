...
import { UPDATE_BANNER } from "../../gql/banners";
...
const UpdateBanner = ({ handleClose, bannerAlert, banner }) => {
  ...
  const [updateBanner] = useMutation(UPDATE_BANNER, {
    ...
    onCompleted: () => {
      ...
      setImagePreview("");
      setLoading(false);
      bannerAlert("The banner have been updated.");
      handleClose();
    },
  });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      updateBanner({ variables: { ...values, id: banner.id } });
    } catch (error) {
      console.log("error : ", error);
    }
  };
