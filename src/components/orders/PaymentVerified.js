...
const PaymentVerified = ({ detailOrder }) => {
    ...
    useEffect(() => {
      if(startDate && endDate) {
        ...
        loadOrdersWithDate({ variables: { limit: rowsPerPage, offset: offset, search: `%${search}%`, status: '%payment_verified%', start: startDate, end: endDate }})
      } else {
        loadOrders({ variables: { limit: rowsPerPage, offset: offset, search: `%${search}%`, status: '%verified%'}})
      }
                  ...
                  <TableRow onClick={() => null} key={index} hover role="checkbox" tabIndex={-1} >
                    ...
                    <TableCell>
                      ...
                      <Button size='small' variant="contained" color="success" onClick={() => {
                        updateStatus({ variables: { id: row.id, status: 'delivering'} })
                      }}>Delivering</Button>
                    </TableCell>
                  </TableRow>
                ))
