...
const PaymentVerified = ({ detailOrder }) => {
    ...
    useEffect(() => {
      if(search && !isNaN(search)) {
        setStartDate(null)
        loadOrderByID({ variables: { search: (search - 1 + 1), status: '%verified%'} })
        return
      }
      if(startDate && endDate) {
        ...
        loadOrdersWithDate({ variables: { limit: rowsPerPage, offset: offset, search: `%${search}%`, status: '%verified%', start: startDate, end: endDate }})
      } else {
