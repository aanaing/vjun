...
import { Breadcrumbs, Typography, Box, CardContent, CardMedia, ListItem, ListItemText, CardActions, Button,
  Modal, Alert,
} from '@mui/material'
...
const Order = () => {
    ...
    const [ update ] = useMutation(UPDATE_ORDER_STATUS, {
      ...
      onCompleted: () => {
        handleClose()
        setShowAlert({ message: 'Order\'s status have been changed to cancelled.', isError: false })
        setTimeout(() => {
            setShowAlert({ message: '', isError: false })
        }, 2000)
      }
    ...
    return (
        <div>
          ...
          <Typography variant='h4' component='h2' sx= {{ my: 2, fontWeight: 'bold' }} >{order.order_status[0].toUpperCase()+order.order_status.slice(1,order.order_status.length)} Order</Typography>
            <Box
                sx={{
                display: 'flex',
                flexFlow: 'wrap row',
              ...
              <Box >
                  <Box>
                      <CardContent sx={{ display: 'flex', paddingInline: '0' }}>
                        {
                          (order.payment_screenshot_image_url && order.payment_screenshot_image_url !== 'null' ) && <CardMedia sx={{ flex: 1}}
                            component="img"
                            height="194"
                            ...
                            <Box sx={{ flex: 1 }}>
                              ...
                              <ListItem>
                                <ListItemText
                                  ...
                                  primaryTypographyProps={{ fontWeight: 'bold' }}
                                ><Link to={`/user/${order.user.id}`}>User's name</Link></ListItemText>
                              </ListItem>
                              <ListItem>
                                <ListItemText
                                  primary="Status"
                              ...
                              <ListItem>
                                <ListItemText
                                  primary="Address"
                                  secondary={`${JSON.parse(order.address).phone}, ${JSON.parse(order.address).address}`}
                                  primaryTypographyProps={{ fontWeight: 'bold' }}
                                />
                              </ListItem>
                            </Box>
                    ...
                    >
                      <Box sx={style}>
                        ...
                        <Box sx={{ textAlign: 'right', mt: 2 }}>
                          ...
                          <Button onClick={() => update({ variables: { id: order.id, status: 'cancelled'} })} >Confirm</Button>
                        </Box>
                      </Box>
                  </Modal>
              </Box>
