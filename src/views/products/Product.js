...
const Product = ({ homeAlert }) => {
  ...
  const handleMoreUpload = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!fileTypes.includes(img.type)) {
        setShowAlert({
          message: "Please select image. (PNG, JPG, JPEG, GIF, ...)",
          isError: true,
        });
        setTimeout(() => {
          setShowAlert({ message: "", isError: false });
  ...
  const [actionProduct] = useMutation(ACTION_PRODUCT, {
    onError: () => {
      setShowAlert({ message: "Error on server", isError: true });
      setTimeout(() => {
        setShowAlert({ message: "", isError: false });
    ...
    onCompleted: () => {
      homeAlert("Succssfully changed.", false);
    },
    refetchQueries: [PRODUCT_BY_ID],
  });
  ...
  const handleAction = () => {
    ...
    //   setShowAlert({
    //     message: "Please, make sure all of its variations have been removed.",
    //     isError: true,
    //   });
    //   setTimeout(() => {
    //     setShowAlert({ message: "", isError: false });
    ...
    //   setShowAlert({
    //     message: "Please, make sure all of its images have been removed.",
    //     isError: true,
    //   });
    //   setTimeout(() => {
    //     setShowAlert({ message: "", isError: false });
    ...
    actionProduct({ variables: { id: product.id, action: !product.disabled } });
  };
    ...
    <div>
      ...
      >
                ...
                <Box>
                  ...
                  <ListItem>
                    <ListItemText
                      primary="Discount"
                      secondary={
                        product.discount_eligible ? "eligible" : "ineligible"
                      }
                    />
                  </ListItem>
                  <ListItem>
                  ...
                  <ListItem>
                    <ListItemText
                      primary="Status"
                      secondary={product.disabled ? "Disabled" : "Enabled"}
                    />
                  </ListItem>
                </Box>
              </Box>
          ...
          >
            <Box sx={styleP}>
              ...
              <Box sx={{ textAlign: "right", mt: 2 }}>
                <Button color="secondary" onClick={handleCloseRI}>
                  Cancel
                </Button>
                <Button
                  onClick={() => {
          ...
          >
            <Box sx={styleP}>
              <Typography
                ...
                component="h2"
              >
                {product.disabled ? "Enable" : "Disable"} Product
              </Typography>
              <Typography id="keep-mounted-modal-description" sx={{ mt: 2 }}>
                Are you sure want to {product.disabled ? "enable" : "disable"}?
                {/* And please, make sure all of its variations and images are removed */}
              </Typography>
              <Box sx={{ textAlign: "right", mt: 2 }}>
        ...
        <ProductVariationTable
          ...
          variationsProp={product?.product_variations}
        />
        <div style={{ minHeight: "auto" }}>
          <Modal
            open={open}
